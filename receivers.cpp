	...
	worker=QtConcurrent::run([this,dataPath]() {
		...
		for (int threadID=0; threadID < threads; threadID++)
		{
			futures.addFuture(QtConcurrent::run([&object,&keys,size=segmentSize.quot,threadID,&segments]() {
				...
				for (int index=segmentStartIndex; index < segmentEndIndex; index++)
				{
					segments[threadID].push_back(std::make_pair(keys.at(index),object.value(keys.at(index)).toString()));
				}
			}));
		}
		...
		for (int index=objectSize-segmentSize.rem; index < objectSize; index++)
		{
			emoticons[keys.at(index)]=object.take(keys.at(index)).toString();
		}
		object=QJsonObject();
		for (int threadID=0; threadID < threads; threadID++)
		{
